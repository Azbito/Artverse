...
export default function ArtCategories() {
  ...
  return (
    <div className="flex justify-center items-center flex-col mt-24">
      ...
      <div className="grid grid-cols-4 gap-16 mt-10 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5  ">
        {arts.map((item) => (
          <div key={item.id} onClick={() => alert("This project was made to understand local storage. There's no more pages.")}>
            <img src={item.img} alt={item.alt} className="h-[10rem] w-[10rem] rounded-lg cursor-pointer hover:scale-125 hover:shadow-lg duration-200 2xl:h-[15rem] 2xl:w-[15rem] lg:w-[10rem] md:w-full" />
